...
$TMP_CELL_PARAMETERS
EOFSCF
local PW="$TMP_PARA_PREFIX $TMP_BIN_DIR/pw.x $TMP_PARA_POSTFIX $TMP_PW_POSTFIX $TMP_SCF_POSTFIX"
...
local PW="$TMP_PARA_PREFIX $TMP_BIN_DIR/pw.x $TMP_PARA_POSTFIX $TMP_NSCF_POSTFIX"
$PW < $INPUT > $OUTPUT || error_handler
...
# Check for spin
local SPIN=`echo "$TMP_SPIN" | grep nspin | sed 's/nspin/nspin_ham/' | sed -e "s/start.*//"`
cat > $INPUT <<EOFHAM
&input
